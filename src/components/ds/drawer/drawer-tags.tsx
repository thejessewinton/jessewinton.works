...
export const DrawerTags = ({
  ...
  const items: TagItem[] = React.useMemo(() => {
    const existing = tags.map((tag) => ({ label: tag, value: tag }))
    const trimmed = inputValue.trim().toLowerCase()

    if (
      trimmed &&
      !tags.some((t) => t.toLowerCase() === trimmed)
    ) {
      existing.push({ label: `Create "${inputValue.trim()}"`, value: inputValue.trim() })
    }
  ...
  const handleValueChange = (values: TagItem[]) => {
    onTagsChange(values.map((v) => v.value))
  }
